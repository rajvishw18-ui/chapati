...
function MenuPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 pt-6 sm:px-6">
      <header className="clay gold-frame rounded-4xl bg-card px-6 py-8 text-center">
        <p className="font-display text-sm font-semibold tracking-[0.3em] text-primary uppercase">
          {t("hotelKind")}
        </p>
        <h1 className="font-script mt-2 text-5xl leading-none text-primary sm:text-7xl">
          {hotel.name}
        </h1>
        <p className="mt-3 text-sm text-muted-foreground">{t("tagline")}</p>
        <div className="mt-5 flex flex-wrap items-center justify-center gap-3">
          <span className="clay-sm rounded-full bg-secondary px-4 py-2 text-xs font-bold text-secondary-foreground">
            {t("pureVeg")}
          </span>
          <LanguageSwitcher />
        </div>
      ...
      <div className="sticky top-3 z-30 mt-5 space-y-3 rounded-4xl bg-background/80 py-3 backdrop-blur-md">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          <FilterPill active={diet === "all"} onClick={() => setDiet("all")}>
            {t("all")}
          </FilterPill>
          {diets.map((d) => (
            <FilterPill key={d} active={diet === d} onClick={() => setDiet(d)}>
        ...
        <div className="mt-6 space-y-10">
          {visibleCategories.map((cat) => (
            <section key={cat.id} id={cat.id}>
              <h2 className="clay-sm mb-4 inline-block rounded-2xl bg-secondary px-5 py-2 text-xl font-extrabold text-secondary-foreground sm:text-2xl">
                {cat.label[lang]}
              </h2>
